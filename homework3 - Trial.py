...
def main():
        ...
        for c_batch_size in m_batch_size:
            for c_hidden_layer_sizes in m_hidden_layer_sizes:
                for c_learning_rate_init in m_learning_rate_init:
                    ...
                    model.fit(sample_inputs[:20000], expected_outputs[:20000])
    
                    # Cross-validate the model
                    np.set_printoptions(threshold=np.nan)
                    np.save("training_output", model.predict(sample_inputs[20000:]))

                    loaded_expected_outputs = np.load("expected_output.npy")[20000:]
    ...
    normalized_test_outputs_file = open("normalized_test_output_trial.csv", "w")
